...
public class ShardRedis {
    ...
    private GenericObjectPool.Config config = AbstractPools.DefaultConfig;
     ...
     * @param isPoolable
     */
    public ShardRedis(String hosts, int timeout, Selector selector, boolean isPoolable) {
        super();
        this.isPoolable = isPoolable;
        init(hosts, timeout, selector.getPloy(), null);
    }

    public ShardRedis(String hosts, int timeout, Selector selector, GenericObjectPool.Config config) {
        super();
        this.isPoolable = true;
        this.config = config;
    ...
    public ShardRedis(String hosts, int timeout, Ploy ploy, GenericObjectPool.Config config) {
        super();
        this.isPoolable = true;
        this.config = config;
    ...
    private void init(String hosts, int timeout, Ploy ploy, GenericObjectPool.Config config) {
        String[] allhosts = hosts.split(" ");
        pools = new JedisPools(config);
    ...
    public RedisComand createRedisCommand(int rw) {
        ...
        return (RedisComand) Proxy.newProxyInstance(
    ...
    private static Jedis jedisConnect(InstanceInfo ShardInfo) throws JedisConnectionException {
        try {
            ...
            throw new JedisConnectionException("Can't connect host:" + ShardInfo.getHost() + ":"
                    + ShardInfo.getPort(), e);
        }
