...
import fengfei.shard.ShardException;
...
public class HashSelector implements Selector {
    ...
    @Override
    public InstanceInfo select(String key, int readWrite) {
        try {
            ...
            if (info == null) {
                throw new Exception("can't find instance for key: " + key);
            }
            return info;
        } catch (Exception e) {
    ...
    @Override
    public InstanceInfo selectMaster(String key) {
    ...
    @Override
    public InstanceInfo selectSlave(String key) {
        return select(key, Read);

    }

    @Override
    public InstanceInfo selectAny(String key) {
